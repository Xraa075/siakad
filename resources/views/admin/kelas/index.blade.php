<div class="container mt-4">
    @if ($jurusanContext)
        ...
        <a href="{{ route('admin.jurusan.index', ['departemen_id' => $jurusanContext->departemen_id]) }}"
            class="btn btn-sm btn-outline-secondary mb-3">&larr; Kembali ke Daftar Jurusan
            {{ $jurusanContext->departemen->nama_departemen }}</a>
    @else
    ...
    @if ($jurusanContext)
        <a href="{{ route('admin.kelas.create', ['jurusan_id' => $jurusanContext->id]) }}" class="btn btn-success mb-3">
            Tambah Kelas (untuk {{ $jurusanContext->nama_jurusan }})
        </a>
